...
from claims.vault_scanner_utils import (
    _keccak_f,
    build_claim_payload,
    build_claim_vault_with_sig_transaction,
    claim_digest,
    claim_message_hash,
    encode_claim_vault_with_sig_calldata,
...
def test_encode_claim_vault_with_sig_calldata_matches_expected_bytes() -> None:
    calldata = encode_claim_vault_with_sig_calldata(
        user="0x1111111111111111111111111111111111111111",
        vault_id="0x" + "22" * 32,
        balance=123,
        attribution_hash="0x" + "33" * 32,
        signature="0x" + "44" * 65,
    )

    # Selector derived from keccak256("claimVaultWithSig(address,bytes32,uint256,bytes32,bytes)")
    assert calldata[:4].hex() == "3bb71497"
    # Five 32-byte words in the head plus the dynamic signature tail (length prefix + 65-byte payload)
    assert len(calldata) == 4 + 32 * 5 + 32 + 96


def test_build_claim_vault_with_sig_transaction_formats_fields() -> None:
    transaction = build_claim_vault_with_sig_transaction(
        user="0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        vault_id="0x" + "bb" * 32,
        balance=500,
        attribution_hash="0x" + "cc" * 32,
        signature="0x" + "dd" * 65,
    ...
    assert transaction["to"] == "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"
    assert transaction["from"] == "0xffffffffffffffffffffffffffffffffffffffff"
    assert transaction["chainId"] == 1337
    assert transaction["gas"] == 210000
    assert transaction["nonce"] == 7
    assert transaction["value"] == "0x0"
    assert transaction["data"].startswith("0x3bb71497")
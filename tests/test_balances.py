...
class _FakeInfo:
    ...
    def user_state(self, **kwargs: object) -> dict[str, object]:  # noqa: D401 - simple stub
...
def test_cli_outputs_human_readable(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    fake_info = _FakeInfo(payload)
    monkeypatch.setattr(cli, "Info", lambda skip_ws=True: fake_info)

    assert cli.main(["0xabc"]) == 0
    out = capsys.readouterr().out
    assert "Withdrawable balance" in out
    assert "$42.50" in out
    assert fake_info.last_call_kwargs == {"address": "0xabc"}
...
def test_cli_forwards_dex_when_supplied(monkeypatch: pytest.MonkeyPatch) -> None:
    import scripts.find_balance as cli

    payload = {"clearinghouseState": {"withdrawable": "12"}}
    fake_info = _FakeInfo(payload)
    monkeypatch.setattr(cli, "Info", lambda skip_ws=True: fake_info)

    assert cli.main(["0xabc", "--dex", "vault"]) == 0
    assert fake_info.last_call_kwargs == {"address": "0xabc", "dex": "vault"}

...
def _encode_uint256(value: int, *, label: str) -> bytes:
    ...
    return value.to_bytes(ABI_WORD_SIZE, "big")


def _encode_address(value: bytes, *, label: str) -> bytes:
    if len(value) != 20:
        raise ValueError(f"{label} must be 20 bytes (got {len(value)} bytes)")
    return b"\x00" * 12 + value


def _encode_dynamic_bytes(value: bytes, *, label: str) -> bytes:
    length_prefix = _encode_uint256(len(value), label=f"{label} length")
    padded_length = ((len(value) + (ABI_WORD_SIZE - 1)) // ABI_WORD_SIZE) * ABI_WORD_SIZE
    return length_prefix + value.ljust(padded_length, b"\x00")
...
def encode_claim_vault_with_sig_calldata(
    user: str,
    vault_id: str,
    balance: int,
    ...
    """Return the ABI-encoded calldata for ``claimVaultWithSig``.

    This helper performs a minimal subset of the Solidity ABI encoding logic so the
    call data can be produced without requiring ``web3`` or ``eth_abi``.  Validation
    mirrors the :func:`build_claim_payload` helpers to surface descriptive errors
    when malformed inputs are provided.
    """

    address_bytes = _decode_hex(user, expected_bytes=20, label="user address")
    vault_bytes = _decode_hex(vault_id, expected_bytes=32, label="vaultId")
    attribution_bytes = _decode_hex(attribution_hash, expected_bytes=32, label="attributionHash")
    signature_bytes = _decode_hex(signature, expected_bytes=65, label="signature")

    selector = _function_selector("claimVaultWithSig(address,bytes32,uint256,bytes32,bytes)")

    head: List[bytes] = [
        _encode_address(address_bytes, label="user address"),
        vault_bytes,
        _encode_uint256(balance, label="balance"),
        attribution_bytes,
        _encode_uint256(ABI_WORD_SIZE * 5, label="signature offset"),
    ]

    tail = _encode_dynamic_bytes(signature_bytes, label="signature")
    return selector + b"".join(head) + tail


def build_claim_vault_with_sig_transaction(
    *,
    user: str,
    vault_id: str,
    balance: int,
    ...
    """Return a dictionary describing the ``claimVaultWithSig`` transaction."""

    calldata = encode_claim_vault_with_sig_calldata(user, vault_id, balance, attribution_hash, signature)
    contract_bytes = _decode_hex(contract_address, expected_bytes=20, label="contract address")
    transaction: Dict[str, object] = {
        "to": "0x" + contract_bytes.hex(),
        "data": "0x" + calldata.hex(),
        "value": "0x0",
    }
    if sender is not None:
        sender_bytes = _decode_hex(sender, expected_bytes=20, label="sender address")
        transaction["from"] = "0x" + sender_bytes.hex()
...
def keccak256(data: bytes) -> bytes:
    ...
    # Absorb full-rate blocks directly from the input.
    data_len = len(data)
    offset = 0
    ...
    # Absorb the final padded block (always processed, even for empty input).
